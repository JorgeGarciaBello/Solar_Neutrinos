subroutine ppChainSolarOsc(t12,t23,t13,delta,sm,aM,nu)
    implicit none
    ...
    real(8) :: PP(1219)    ! F17_solar_zone is the 8boro created inthe solar zones
    real(8) :: PP_e_sptrm(84,2) ! Neutrino energy spectrum from Boro-8 [MeV]
    ...
    real(8) :: neuPP             ! is the amount of neutrinos from de Boron with energy j and from the xone k 
    real(8) :: neuPPAllEnergy     ! is the total number of neutrinos in the solar zone "zone"
    real(8) :: neuPPAllZoneEnergy ! is the total number of neutrinos in the sun bye energy and zone
    ...
    !print*, 'PP_e_sptrmm',PP_e_sptrm(:,:)
    !print*, 'PP_e_sptrmm',PP_e_sptrm(:,2)
    ...
    dimen=shape(PP_e_sptrm);
    energy_step=dimen(1)
    zone_step=size(PP)
    
    open(21,file='elements_solar_oscillations_model/pp_chain/pp_chain_sun_oscillation_core.dat')
    open(22,file='elements_solar_oscillations_model/pp_chain/pp_chain_sun_llimit_earth_oscillation_core.dat')
    open(23,file='elements_solar_oscillations_model/pp_chain/pp_chain_sun_earth_oscillation_core.dat')
    neuPPAllZoneEnergy=0.0D0
    do zone=1,zone_step
        neuPPAllEnergy=0.0D0 ! is the total number of neutrinos in the solar zone "zone"
        do j=1,energy_step
            energy=PP_e_sptrm(j,1)
            if(energy.Gt.0.0d0) then
                ! PP(k)  Neutrinos generados en la zona K
                ! PP_e_sptrm(j,2)   Neutrinos qgenerados con energía (j.2)
                !print*,'PP(zone)*PP_e_sptrm(j,2)',j,PP(zone),PP_e_sptrm(j,2)
                neuPP=PP(zone)*PP_e_sptrm(j,2) ! Neutrinos generados en la zona "zone" y energía "energy-j"
                ...
                neuPPAllEnergy=neuPPAllEnergy+neuPP*totalProbability                
                if (zone==1) then
        ...
        enddo
        neuPPAllZoneEnergy=neuPPAllZoneEnergy+neuPPAllEnergy        
        print*,'neuPPAllZoneEnergy',neuPPAllZoneEnergy
    enddo
    ...
    return
end subroutine ppChainSolarOsc