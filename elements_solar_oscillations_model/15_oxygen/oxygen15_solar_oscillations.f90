subroutine oxygen15SolarOsc(t12,t23,t13,delta,sm,aM,nu)
    implicit none
    ...
    real(8) :: Oxygen15(1219)    ! F17_solar_zone is the 8boro created inthe solar zones
    real(8) :: O15_e_sptrm(200,2) ! Neutrino energy spectrum from Boro-8 [MeV]
    ...
    real(8) :: neuO15             ! is the amount of neutrinos from de Boron with energy j and from the xone k 
    real(8) :: neuO15AllEnergy     ! is the total number of neutrinos in the solar zone "zone"
    real(8) :: neuO15AllZoneEnergy ! is the total number of neutrinos in the sun bye energy and zone
    ...
    !print*, 'Oxygen15', Oxygen15
    call readO15SolarNeutrinoEnergySpectrum(O15_e_sptrm)
    !print*, 'O15_e_sptrmm',O15_e_sptrm(:,:)
    !print*, 'O15_e_sptrmm',O15_e_sptrm(:,2)
    ...
    dimen=shape(O15_e_sptrm);
    energy_step=dimen(1)
    zone_step=size(Oxygen15)
    
    open(21,file='elements_solar_oscillations_model/15_oxygen/15_oxygen_sun_oscillation_core.dat')
    open(22,file='elements_solar_oscillations_model/15_oxygen/15_oxygen_sun_llimit_earth_oscillation_core.dat')
    open(23,file='elements_solar_oscillations_model/15_oxygen/15_oxygen_sun_earth_oscillation_core.dat')
    neuO15AllZoneEnergy=0.0D0
    do zone=1,zone_step
        neuO15AllEnergy=0.0D0 ! is the total number of neutrinos in the solar zone "zone"
        do j=1,energy_step
            energy=O15_e_sptrm(j,1)
            if(energy.Gt.0.0d0) then
                ! Oxygen15(k)  Neutrinos generados en la zona K
                ! O15_e_sptrm(j,2)   Neutrinos qgenerados con energía (j.2)
                !print*,'Oxygen15(zone)*O15_e_sptrm(j,2)',j,Oxygen15(zone),O15_e_sptrm(j,2)
                neuO15=Oxygen15(zone)*O15_e_sptrm(j,2) ! Neutrinos generados en la zona "zone" y energía "energy-j"
                ...
                neuO15AllEnergy=neuO15AllEnergy+neuO15*totalProbability                
                if (zone==1) then
        ...
        enddo
        neuO15AllZoneEnergy=neuO15AllZoneEnergy+neuO15AllEnergy        
        print*,'neuO15AllZoneEnergy',neuO15AllZoneEnergy
    enddo
    ...
    return
end subroutine oxygen15SolarOsc